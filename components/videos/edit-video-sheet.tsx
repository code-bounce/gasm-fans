...
export function EditVideoSheet({ video }: EditVideoSheetProps) {
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [modelSearch, setModelSearch] = useState("");
  const [selectedModelIds, setSelectedModelIds] = useState<string[]>(
    video.models?.map((m) => m.id) || [],
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setOpen(false);
        // Invalidate both specific video and videos list
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="max-h-screen overflow-y-auto px-5">
        <SheetHeader className="px-0">
          ...
          <SheetDescription>Update the video details below.</SheetDescription>
        </SheetHeader>
